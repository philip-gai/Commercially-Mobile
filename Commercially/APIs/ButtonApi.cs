...
namespace Commercially
{
	public static class ButtonApi
	{
		...
		{
			var resp = HttpRequest.MakeRequest(HttpRequestMethodType.PATCH, Url + buttonId, body, "Bearer " + SessionData.OAuth.access_token);
			return resp;
		}

		public static string PairButton(string buttonId, string clientId)
		{
			var resp = HttpRequest.MakeRequest(HttpRequestMethodType.POST, Url + buttonId + "/pair", new JObject("client_id", clientId).ToString(), "Bearer " + SessionData.OAuth.access_token);
			return resp;
		}
	}
}